...
logger.info('The Code Hastebin Panel! Bu panel Open Coders a aittir. ');
...
for (var name in config.documents) {
	...
	if (data) {
		...
		logger.verbose('Oluşturulan döküman: ' + name + " ==> " + path);
	} else {
		logger.warn('Döküman bulunamadı: ' + name + " ==> " + path);
	}
}
...
logger.info('Dinlenen Port: ' + config.host + ':' + config.port);
